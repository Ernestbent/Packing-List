...
@frappe.whitelist()
def create_courier_details(sales_order, first_name, tel_no, vehicle_no, packing_list=None, box_rows=None):

    frappe.log_error(f"create_courier_details called with sales_order: {sales_order}")

    if not first_name or not tel_no or not vehicle_no:
