## Evening Notification: Pending Credit Approval Orders Summary
## Schedule: 6:00 PM daily

import frappe
from frappe.utils import get_url

def send_evening_pending_orders_notifications():
    ## Get default sender email
    sender_email = frappe.db.get_value("Email Account", {"default_outgoing": 1}, "email_id")
    if not sender_email:
        frappe.log_error("No sender email configured.", "Evening Notification Failed")
        return
    ...
    if not pending_orders:
        frappe.logger().info("No pending credit approval orders for evening notification")
        return
    ...
    subject = f"EVENING SUMMARY: {len(pending_orders)} Order(s) Awaiting Credit Approval"
    ...
    for email in recipient_emails:
        ...
        <html>
            <body style="font-family: Arial, sans-serif; line-height: 1.6;">
                <p>Good evening,</p>
                <p style="color:#333;">
                    End-of-day summary: <strong>{len(pending_orders)} order(s)</strong> currently awaiting credit approval.
                </p>
                
                <p style="font-weight: bold; margin-top: 20px;">Pending Orders:</p>
                ...
                <p style="margin-top: 20px; color:#666; font-size: 12px;">
                    This is an automated daily summary. Please review pending orders and take necessary action.
                </p>
                <br>
        ...
        try:
            ...
            frappe.logger().info(f"Evening notification sent to {email}")
        except Exception as e:
            frappe.log_error(
                f"Failed to send evening notification to {email}: {str(e)}",
                "Evening Notification Error"
            )
