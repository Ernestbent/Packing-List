...
def get_data(filters):
    ...
    # Organize prices by item and price list
    price_map = {}
    ...
    for item in items:
        ...
        row["verma_price"] = verma_price if verma_price else None
        ...
        row["grand_total"] = grand_total if grand_total > 0 else None
        
        # Filter out items with no prices if option selected
...
def round_up_to_next_thousand(amount):
    """Round up to next thousand"""
    if not amount or amount <= 0:
        return 0
    return int(frappe.utils.ceil(amount / 1000) * 1000)