...
def get_data(month, year, num_days):
    ...
    packing_rows = [{"person": resolve_name(r["person"], user_name_map),
                     "activity": "Packing", "day_num": r["day_num"], "qty": r["qty"]}
                    for r in packing_orders_raw]

    ## PICKING ORDERS: count distinct Sales Orders picked per person per day
    ...
    verifier_orders_raw = frappe.db.sql("""
        SELECT person, custom_date, custom_sales_order
        FROM (
            -- Verifier 1: the person who submitted the Packing List
            SELECT owner AS person, custom_date, custom_sales_order
            FROM `tabPacking List`
            WHERE docstatus = 1
                AND MONTH(custom_date) = %(month)s
                AND YEAR(custom_date)  = %(year)s
            ...
            SELECT custom_verifier_2 AS person, custom_date, custom_sales_order
            FROM `tabPacking List`
            WHERE docstatus = 1
                AND MONTH(custom_date) = %(month)s
                AND YEAR(custom_date)  = %(year)s
    ...
    ## Deduplicate by (resolved_name, sales_order) — avoids double count when V1 == V2
    seen       = set()
    verify_map = {}
    for r in verifier_orders_raw:
        name      = resolve_name(r["person"], user_name_map)
        so        = r["custom_sales_order"]
        day_num   = frappe.utils.getdate(r["custom_date"]).day
        dedup_key = (name, so)

        ## Skip if this person already counted for this SO
        if dedup_key in seen:
            continue
        seen.add(dedup_key)

        key = (name, day_num)
        verify_map[key] = verify_map.get(key, 0) + 1
    ...
    for person, keys in persons_keys.items():
        ...
        for i, key in enumerate(keys):
            ...
            ## Total = sum of all day columns for this activity row
            ...
            ## Each summary column shows its own activity total — None on all other rows
            ...
            ## Combined totals cleared here — filled on last row below
        ...
        ## Combined summary placed on last row per person
        if person_rows:
